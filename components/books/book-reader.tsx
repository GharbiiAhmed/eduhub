...
export function BookReader({ pdfUrl, title, open, onOpenChange }: BookReaderProps) {
  ...
  const requestFlip = (dir: FlipDir) => {
    ...
    // allow paper to cover repaint
    setTimeout(() => {
      setPageIndex(next)
      setIsFlipping(false)

      // fade paper out AFTER render
      setTimeout(() => setShowPaper(false), 80)
    }, 160)
  }
            ...
            {numPages > 0 && (
              <div className="br-bookWrap">
                <div className={`br-book ${isTwoPage ? "two" : "one"}`}>
                  ...
                  {isFlipping && (
                    <div
                      className={`br-flipSheet flip-${flipDir}`}
                      style={{
                        width: pageWidth,
                  ...
                  {/* Paper mask */}
                  {showPaper && <div className="br-paperOverlay" />}
                </div>
              </div>
            )}
          </Document>
