...
export default function InstructorHelpCenterPage() {
  ...
  const fetchArticles = async () => {
    try {
      ...
      toast({
        title: tCommon('error'),
        description: t('failedToFetchArticles'),
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({ error: `HTTP error! status: ${response.status}` }))
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-2">
                {t('manageHelpArticlesForCoursesSection')}
              </p>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>{t('order')}: {article.order_index}</span>
                        </div>
                      </div>
