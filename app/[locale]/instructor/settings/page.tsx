    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-6">
        <Avatar className="w-24 h-24">
          ...
          <AvatarFallback className="text-2xl">
            {profileData.full_name?.charAt(0) || 'I'}
          </AvatarFallback>
        </Avatar>
        <div>
      ...
      <div>
        ...
        <Textarea
          ...
          placeholder={t('tellUsAboutYourselfAndExpertise')}
        />
      </div>
    </div>
  )
  ...
  const renderNotificationsTab = () => (
    <div className="space-y-4">
      {Object.entries(userSettings)
        .filter(([key]) => {
          // Include notification-related keys but exclude privacy keys
          const isNotificationKey = key.includes('notification') || 
          ...
          // Exclude privacy keys
          const isPrivacyKey = key.startsWith('show_') || key === 'allow_messages' || key === 'data_sharing'
          return isNotificationKey && !isPrivacyKey
        })
        .map(([key, value]) => (
          <div key={key} className="flex items-center justify-between p-4 border rounded-lg">
            <div>
              <h4 className="font-medium">
                {tNotifications(key as any)}
              </h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            {t('manageInstructorAccountSettings')}
          </p>
        </div>
