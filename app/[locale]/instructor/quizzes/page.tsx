...
export default function QuizManagement() {
  ...
  const handleDeleteQuiz = async (quizId: string) => {
      if (!confirm(tDashboard('confirmDeleteQuiz'))) {
        return
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{tDashboard('quizManagement')}</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            {tDashboard('createAndManageQuizzes')}
          </p>
        </div>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-blue-600 hover:bg-blue-700">
              ...
              {tDashboard('createQuiz')}
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>{tDashboard('createNewQuiz')}</DialogTitle>
              <DialogDescription>
                {tDashboard('setUpNewQuiz')}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="title">{tDashboard('quizTitle')}</Label>
                  <Input
                    id="title"
                ...
                <div>
                  <Label htmlFor="course">{tCommon('course')}</Label>
              ...
              <div>
                <Label htmlFor="module">{t('module')} ({tCommon('optional')})</Label>
                <Select value={formData.module_id} onValueChange={(value) => setFormData({ ...formData, module_id: value, lesson_id: '' })}>
                  ...
                  <SelectContent>
                    <SelectItem value="none">{tDashboard('noSpecificModule')}</SelectItem>
              ...
              <div>
                <Label htmlFor="lesson">{t('lesson')}</Label>
                <Select value={formData.lesson_id} onValueChange={(value) => setFormData({ ...formData, lesson_id: value })}>
                  <SelectTrigger>
                    <SelectValue placeholder={formData.module_id && formData.module_id !== 'none' ? t('selectLesson') : tDashboard('selectModuleFirst')} />
                  </SelectTrigger>
                  <SelectContent>
                    {lessons.map((lesson) => (
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder={tDashboard('enterQuizDescription')}
                  rows={3}
                />
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <Label htmlFor="time_limit">{tDashboard('timeLimitMinutes')}</Label>
                  <Input
                    id="time_limit"
                    type="number"
                    value={formData.time_limit}
                    onChange={(e) => setFormData({ ...formData, time_limit: e.target.value })}
                    placeholder={tDashboard('noLimit')}
                  />
                </div>
                <div>
                  <Label htmlFor="max_attempts">{tDashboard('maxAttempts')}</Label>
                  <Input
                    id="max_attempts"
                    type="number"
                ...
                <div>
                  <Label htmlFor="passing_score">{tDashboard('passingScorePercent')}</Label>
                  <Input
                    id="passing_score"
                    type="number"
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label htmlFor="is_published">{tDashboard('publishImmediately')}</Label>
                  <Switch
                    id="is_published"
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="is_randomized">{tDashboard('randomizeQuestions')}</Label>
                  <Switch
                    id="is_randomized"
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="show_correct_answers">{tDashboard('showCorrectAnswers')}</Label>
                  <Switch
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="show_results_immediately">{tDashboard('showResultsImmediately')}</Label>
                  <Switch
      ...
      <div className="flex items-center space-x-4">
        <div className="flex-1">
          <Input
            placeholder={tDashboard('searchQuizzes')}
            value={searchTerm}
            ...
            <CardHeader>
              <div className="flex items-start justify-between">
                ...
                <Badge variant={quiz.is_published ? "default" : "secondary"}>
                  {quiz.is_published ? tCommon('published') : tCommon('draft')}
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>{quiz.questions_count || 0} {tDashboard('questions')}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Clock className="w-4 h-4 text-green-600" />
                    <span>{quiz.time_limit ? `${quiz.time_limit}m` : tDashboard('noLimit')}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Target className="w-4 h-4 text-purple-600" />
                    <span>{quiz.passing_score}% {tDashboard('toPass')}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Users className="w-4 h-4 text-orange-600" />
                    <span>{quiz.max_attempts} {tDashboard('attempts')}</span>
                  </div>
                </div>
                ...
                {quiz.analytics && (
                  <div className="pt-3 border-t">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-gray-600 dark:text-gray-400">{tDashboard('attempts')}:</span>
                        <span className="ml-1 font-medium">{quiz.analytics.total_attempts}</span>
                      </div>
                      <div>
                        <span className="text-gray-600 dark:text-gray-400">{tCommon('students')}:</span>
                        <span className="ml-1 font-medium">{quiz.analytics.total_students}</span>
                      </div>
                      <div>
                        <span className="text-gray-600 dark:text-gray-400">{tDashboard('avgScore')}:</span>
                        <span className="ml-1 font-medium">{quiz.analytics.average_score.toFixed(1)}%</span>
                      </div>
                      <div>
                        <span className="text-gray-600 dark:text-gray-400">{tDashboard('passRate')}:</span>
      ...
      {filteredQuizzes.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
            {tDashboard('noQuizzesFound')}
          </h3>
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {searchTerm || filterCourse !== 'all' || filterStatus !== 'all'
              ? tDashboard('tryAdjustingFilters')
              : tDashboard('createFirstQuizToGetStarted')}
          </p>
          {(!searchTerm && filterCourse === 'all' && filterStatus === 'all') && (
            <Button onClick={() => setIsCreateDialogOpen(true)}>
              <Plus className="w-4 h-4 mr-2" />
              {tDashboard('createYourFirstQuiz')}
            </Button>
          )}
        </div>
      )}
      ...
      <Dialog open={!!editingQuiz} onOpenChange={() => setEditingQuiz(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{tDashboard('editQuiz')}</DialogTitle>
            <DialogDescription>
              {tDashboard('updateQuizSettings')}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="edit-title">{tDashboard('quizTitle')}</Label>
                <Input
                  id="edit-title"
                  value={formData.title}
                  onChange={(e) => setFormData({ ...formData, title: e.target.value })}
              ...
              <div>
                <Label htmlFor="edit-course">{tCommon('course')}</Label>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <Label htmlFor="edit-time_limit">{tDashboard('timeLimitMinutes')}</Label>
                <Input
                  id="edit-time_limit"
                  type="number"
                  value={formData.time_limit}
              ...
              <div>
                <Label htmlFor="edit-max_attempts">{tDashboard('maxAttempts')}</Label>
                <Input
              ...
              <div>
                <Label htmlFor="edit-passing_score">{tDashboard('passingScorePercent')}</Label>
                <Input
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label htmlFor="edit-is_published">{tCommon('published')}</Label>
                <Switch
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="edit-is_randomized">{tDashboard('randomizeQuestions')}</Label>
                <Switch
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="edit-show_correct_answers">{tDashboard('showCorrectAnswers')}</Label>
                <Switch
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="edit-show_results_immediately">{tDashboard('showResultsImmediately')}</Label>
                <Switch
