...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!session) {
        setError(t('authSessionMissing') || "Auth session missing! Please use the link from your email to reset your password.")
        setIsLoading(false)
        return
      }
  ...
  const passwordRequirements = [
    ...
    { text: t('passwordsMatch'), met: password === confirmPassword && confirmPassword.length > 0 }
  ]
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <p className="text-muted-foreground">{t('validatingResetLink')}</p>
          </div>
        </div>
      </div>
    )
