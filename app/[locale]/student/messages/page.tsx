...
export default function StudentMessagesPage() {
      ...
      const conversationsWithDetails = await Promise.all(
        (conversationsData || []).map(async (conv) => {
          ...
          return {
            ...
            participants: [
              { id: user.id, name: t('you'), email: user.email || '', role: 'student' },
              { 
            ...
            last_message: lastMessage ? {
              ...
              sender_name: senderProfile?.full_name || t('unknown'),
              created_at: lastMessage.created_at
            } : {
              content: t('noMessagesYet'),
              sender_name: '',
    ...
    try {
      ...
      const formattedMessages: Message[] = messagesData.map(msg => {
        ...
        return {
          ...
          sender_name: msg.sender_id === user.id ? t('you') : (senderProfile?.full_name || senderProfile?.email || t('unknown')),
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      alert(`${t('errorSendingMessage')}: ${error?.message || tCommon('error')}`)
    }
  }
        ...
        <Dialog open={showNewConversation} onOpenChange={setShowNewConversation}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {t('sendMessageToInstructorOrStudent')}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
                  ...
                  <SelectContent>
                    {enrolledCourses.length === 0 ? (
                      <div className="px-2 py-1.5">
                        <div className="text-sm text-muted-foreground mb-2">{t('noEnrolledCourses')}</div>
                        <Link href="/courses" className="text-xs text-blue-600 hover:underline">
                          {t('browseCoursesToEnroll')}
                        </Link>
                      </div>
                    ) : (
              ...
              {newConversation.selectedCourse && (
                <div>
                  <label className="block text-sm font-medium mb-2">{t('selectRecipient')}</label>
                  {loadingRecipients ? (
                    <div className="text-sm text-muted-foreground">{t('loading')}</div>
                  ) : courseRecipients.length === 0 ? (
                    <div className="text-sm text-muted-foreground">{t('noRecipientsAvailable')}</div>
                  ) : (
                    <Select
              ...
              <div>
                <label className="block text-sm font-medium mb-2">{t('subjectOptional')}</label>
                <Input
              ...
              {filteredConversations.length === 0 ? (
                <div className="text-center py-12 px-6">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-4">
                    {t('startConversationWithInstructorsOrStudents')}
                  </p>
