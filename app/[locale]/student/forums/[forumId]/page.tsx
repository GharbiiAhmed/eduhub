...
import { Reply, ThumbsUp, MessageSquare, User } from "lucide-react"
...
export default function ForumDetailPage({ params }: { params: Promise<{ forumId: string }> }) {
  ...
  useEffect(() => {
    const fetchForum = async () => {
      ...
      try {
          ...
          if (postsError) {
            ...
            // Add author names, reply counts, and like counts to posts
              const postsWithAuthors = postsData.map((post: any) => {
                ...
                const authorName = profile?.full_name || profile?.email?.split('@')[0] || tCommon('anonymous')
        ...
        setError(err.message || t('failedToLoadForum'))
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user) throw new Error(tCommon('notAuthenticated'))
      ...
      // Get author profile (user is already available from above)
      ...
      // Refresh posts list to get the latest data
  ...
  if (!forum) {
    return <div>{t('forumNotFound')}</div>
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('startDiscussion')}</CardTitle>
          <CardDescription>{t('shareThoughtsWithStudents')}</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleCreatePost} className="space-y-4">
            <div className="grid gap-2">
              ...
              <Input
                id="postTitle"
                placeholder={t('whatsYourQuestionOrTopic')}
                required
                value={newPostTitle}
            ...
            <div className="grid gap-2">
              <Label htmlFor="postContent">{t('postContent')}</Label>
              <Textarea
                id="postContent"
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <CardDescription className="flex items-center gap-2 mt-1">
                      ...
                      <span>{post.author_name || tCommon('anonymous')}</span>
                      <span>•</span>
                      ...
                      <div key={reply.id} className="bg-muted p-3 rounded-lg">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm font-medium">
                            {reply.author_name || tCommon('anonymous')}
                          </span>
