      ...
      <div>
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-white/80 text-sm">{t('booksAvailable')}</div>
                  </div>
                </div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-white/80 text-sm">{t('downloads')}</div>
                  </div>
                </div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger className="w-48">
                <SelectValue placeholder={tCourses('sortBy')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="newest">{tCourses('newestFirst')}</SelectItem>
                <SelectItem value="oldest">{tCourses('oldestFirst')}</SelectItem>
                <SelectItem value="price_low">{tCourses('priceLowToHigh')}</SelectItem>
                <SelectItem value="price_high">{tCourses('priceHighToLow')}</SelectItem>
                <SelectItem value="popular">{tCourses('mostPopular')}</SelectItem>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select value={filterPrice} onValueChange={setFilterPrice}>
                    <SelectTrigger>
                      <SelectValue placeholder={tCourses('allPrices')} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">{tCourses('allPrices')}</SelectItem>
                      <SelectItem value="free">{tCourses('free')}</SelectItem>
                      <SelectItem value="paid">{tCourses('price')}</SelectItem>
                      <SelectItem value="under20">{t('under20')}</SelectItem>
                      <SelectItem value="over20">{t('over20')}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="text-gray-600 dark:text-gray-400">
            {t('showing')} {filteredBooks.length} {t('of')} {books.length} {t('books')}
          </div>
          <div className="flex items-center space-x-2">
            {searchTerm && (
          ...
          <div className="flex-1 p-6">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-2">{t('author')}: {book.author}</p>
              ...
              <div className="ml-6 text-right">
                <div className="text-2xl font-bold text-green-600 mb-2">
                  {book.price === 0 ? tCourses('free') : `$${book.price}`}
                </div>
  ...
  return (
    <Card className="group hover:shadow-xl transition-all duration-300 hover:scale-105">
      ...
      <CardContent className="p-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-sm mb-2">{t('author')}: {book.author}</p>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex space-x-1">
            {book.physical_available && (
              <Badge variant="secondary" className="text-xs">{t('physical')}</Badge>
            )}
